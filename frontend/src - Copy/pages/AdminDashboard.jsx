...
export default function AdminDashboard() {
  ...
  const totalClients = (data.clientsPerLocation || []).reduce((s, l) => s + l.count, 0);
  const totalRatedPros = (data.prosAverageRatingPerLocation || []).reduce((s, l) => s + l.count, 0);
  const totalRatedBookings = (data.bookingsAverageRatingPerLocation || []).reduce((s, l) => s + l.count, 0);
  ...
  return (
    <div className="dashboard">
      ...
      {/* Location Tables with Percentages */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "2rem", marginTop: "1rem" }}>
        <div className="section-card">
          <div className="section-title"><span></span> Users per Location</div>
          <DataTable data={data.usersPerLocation} total={totalUsers} />
        </div>
        <div className="section-card">
          <div className="section-title"><span></span> Pros per Location</div>
          <DataTable data={data.prosPerLocation} total={totalPros} />
        </div>
      </div>
      ...
      <div className="section-card">
        ...
        <DataTable data={data.clientsPerLocation} total={totalClients} />
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "2rem" }}>
        <div className="section-card">
          ...
          <DataTable data={data.requestsPerLocation} total={totalRequests} />
        </div>
        <div className="section-card">
          <div className="section-title"><span></span> Bookings per Category</div>
          <DataTable data={data.bookingsPerCategory} total={totalBookings} />
        </div>
      </div>

      {/* Ratings per Location with Percentages */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "2rem", marginTop: "1rem" }}>
        <div className="section-card">
          <div className="section-title"><span></span> Pro Average Ratings per Location</div>
          <RatingTable data={data.prosAverageRatingPerLocation} total={totalRatedPros} />
        </div>
        <div className="section-card">
          <div className="section-title"><span></span> Booking Average Ratings per Location</div>
          <RatingTable data={data.bookingsAverageRatingPerLocation} total={totalRatedBookings} />
        </div>
      </div>
    </div>
  );
      ...
      <table>
        <thead>
          <tr>
            ...
            <th>Count</th>
            {total !== undefined && <th>Percentage</th>}
          </tr>
        </thead>
        <tbody>
          {data.map((item) => {
            const percentage = total ? ((item.count / total) * 100).toFixed(1) : null;
            return (
              <tr key={item._id}>
...
function RatingTable({ data, total }) {
  if (!data || data.length === 0) {
    return <p className="no-data">No data available</p>;
  }
  return (
      ...
      <table>
        <thead>
          <tr>
            ...
            <th>Count</th>
            {total !== undefined && <th>% of Ratings</th>}
          </tr>
        </thead>
        <tbody>
          {data.map((item) => {
